import { createRoot, useState, useEffect,createHandler, useClient } from 'seniman';
...
const calculateTimeLeft = () => {
  ...
  const clientTime = new Date(nowUTC.getTime() - clientWaktu);
...
function MyComponent() {
  ...
  let handleTimestamp = createHandler((timestamp) => {
    clientWaktu = timestamp
    console.log(timestamp );
  });

  useEffect(() => {
    let sendCurrentTimestamp = $c(() => {
...
console.log(timezoneOffset)
      ...
      $s(handleTimestamp)(timezoneOffset);
    });

    client.exec(sendCurrentTimestamp);
  }, []); 
  return (
    <div>Timezone Sent</div>
  );
}
